...
TEST_PARAMS=""
TEST_PROMPT_PRE="Please plug the OTG cable and USB devices into CN9"

function Test_Function
{
	#echo "Please plug the OTG cable and USB devices into CN9"
	echo ""
	HUB4P_BUS_NUM=`lsusb -t | grep -B1 "hub/4p" |grep Bus |awk '{print $3}' |cut -c 2-2`
	#BUS_NUM=1
	if [ $HUB4P_BUS_NUM == 1 ]; then
		BUS_NUM=2 
	elif [ $HUB4P_BUS_NUM == 2 ]; then
		BUS_NUM=1
	else 
